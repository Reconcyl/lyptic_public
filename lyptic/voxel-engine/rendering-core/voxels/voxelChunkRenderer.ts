...
export interface IVoxelMaterialProvider<TChunkWrapper extends IVoxelChunkDataWrapper<TChunkWrapper, TVoxel>, TVoxel> {
    ...
    isVoxelSolid(pointer: VoxelChunkPointer<TChunkWrapper, TVoxel> | null): boolean;
}
...
export class VoxelChunkRenderer {
     ...
     * NOTE: The buffer passed to this utility will be automatically bound.
     ...
     * Relies upon:
     * - Voxel rendering program must be bound
     * - Texture info (frame count & texture unit)
     * - attrib_vertex_data vertex attrib must be enabled
     * - Rendering matrices
     * - Depth test and cull face (CCW) should be enabled
     * Modifies the following GL registers:
     * - The bound buffer
     * - VAP configs
     * @param chunk_pos: The position of the chunk in chunk world space.
     */
    render(gl: GlCtx, program: VoxelRenderingShaderInfo, chunk_pos: vec3) {
    ...
    private uploadNewFaces(gl: GlCtx, faces_to_add: FaceToAdd[]) {
        ...
        const face_elements_buffer = new Uint16Array(faces_to_add.length * 12);
        {
            ...
            for (const added_face of faces_to_add) {
                ...
                face_definition.axis.appendQuadVoxelData(face_elements_buffer, face_origin_idx,
     ...
     * Uses the material provider to determine whether or not a voxel is solid.
     * NOTE: Must be called after the chunk data is modified to reflect all the changes.
     * NOTE: The chunk's data must be inside the world for cross chunk boundaries to properly work.
     * @param gl: The WebGl context
     ...
     * @param material_provider: The material provider used to shade the voxels and determine their model.
     */
    handleVoxelModifications<TChunkWrapper extends IVoxelChunkDataWrapper<TChunkWrapper, TVoxel> & IVoxelChunkRendererWrapper, TVoxel>(gl: GlCtx, chunk: TChunkWrapper, modified_locations: Iterable<vec3>, material_provider: IVoxelMaterialProvider<TChunkWrapper, TVoxel>) {  // TODO: Add support for slabs and multiple passes.
        ...
        const faces_to_add: FaceToAdd[] = [];
        {
            ...
            for (const root_vec of modified_locations) {
                ...
                const root_now_solid = material_provider.isVoxelSolid(root_pointer);

                for (const face_definition of FACES_LIST) {
                    const neighbor_pointer = root_pointer.getNeighbor(face_definition);
                    const neighbor_is_solid = material_provider.isVoxelSolid(neighbor_pointer);
    ...
    reShadeFace<TChunkWrapper extends IVoxelChunkDataWrapper<TChunkWrapper, TVoxel> & IVoxelChunkRendererWrapper, TVoxel>(gl: GlCtx, target_voxel_ptr: VoxelChunkPointer<TChunkWrapper, TVoxel>, voxel_face: FaceDefinition, material_provider: IVoxelMaterialProvider<TChunkWrapper, TVoxel>): boolean {
        ...
        voxel_face.axis.appendQuadVoxelData(new_face_data, 0,
    ...
    /**
     * @desc Forces the buffer to be resized to its ideal size.
     * NOTE: This method expects this chunk's buffer to already be bound to the ARRAY_BUFFER.
     */
    forceResize(gl: GlCtx) {
        this.face_set_manager.resizeCapacity(gl);
    }

    /**
     * @desc Returns the discrepancy between ideal buffer capacity and actual buffer capacity.
     * Relay to GpuSetBuffer.getIdealSizeDiscrepancy()
     */
    getIdealSizeDiscrepancy() {
        return this.face_set_manager.getIdealSizeDiscrepancy();
    }

    /**
     * @desc Frees the voxel mesh buffer of this chunk.
     * Note: CPU mirrored data will still exist but this VoxelChunkRenderer will no longer work properly.
     */
    free(gl: GlCtx) {
