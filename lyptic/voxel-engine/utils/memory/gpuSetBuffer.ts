...
export class GpuSetBuffer {
     ...
     * @param buffer_capacity: The current capacity of the buffer in bytes.
    ...
    /**
     * @desc Returns the discrepancy between ideal buffer capacity and actual buffer capacity IN BYTES.
     * Positive if the buffer should be larger and negative if the buffer should be smaller.
     */
    getIdealSizeDiscrepancy() {
        return GpuSetBuffer.getIdealCapacityBytes(this.elem_byte_size, this.element_count, this.get_ideal_capacity);
    }
}